...
interface AsButtonProps extends SharedProps, ButtonHTMLAttributes<HTMLButtonElement> {
...
interface AsLinkProps extends SharedProps, Omit<InertiaLinkProps, 'href' | 'as'> {
...
export default function Button({
  ...
  const baseStyles = 'inline-block text-center text-base font-brand font-bold text-white px-10 py-3 rounded-[10rem]'
  ...
  if ('href' in props && props.href !== undefined) {
    ...
    return (
      <Link {...(props as AsLinkProps)} as={isGet ? 'a' : 'button'} type={isGet ? undefined : 'button'} className={combinedClass}>
        {children}
      </Link>  
    )
  }
  ...
  return (
    <button {...(props as AsButtonProps)} type={props.type || 'button'} className={combinedClass}>
      {children}
    </button>
  )
}